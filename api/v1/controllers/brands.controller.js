...
const brandService = require(path.join(__dirname, '../services/brand.service'));
...
function createSchema(req, res, next) {
	const schema = Joi.object({
		...
		numberOfAvailableSlots: Joi.string().required(),
    });
	validateRequest(req, next, schema);
}

function create(req, res, next) {
	brandService.create(req.body, req.user.id)
...
function updateSchema(req, res, next) {
	const schemaRules = Joi.object({
		description: Joi.string().required(),
		name: Joi.string().required(),
		numberOfAvailableSlots: Joi.number().required(),
	});
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		brandService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		brandService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        brandService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		brandService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
