...
const schoolCertificateService = require(path.join(__dirname, '../services/school-certificate.service'));
...
router.put('/:id/update-schoolCertificate-status', authorize([Role.Admin,Role.Client, Role.ProjectManager]), updateProgressStatusSchema, updateProgressStatus);
...
function getAll(req, res, next) {
	schoolCertificateService.getAll()
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        schoolCertificateService.getById(req.params.id)
...
function create(req, res, next) {
	schoolCertificateService.create(req.body, req.user.id)
...
function updateSchema(req, res, next) {
	...
	if (req.user.role === Role.Admin) {
        schemaRules.role = Joi.string().valid(
			schoolCertificateService.schoolCertificateCreated,
			schoolCertificateService.schoolCertificateResolved,
			schoolCertificateService.schoolCertificateApproved,
			schoolCertificateService.schoolCertificateInProgress
		).empty('');
		}
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		schoolCertificateService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateProgressStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		schoolCertificateService.updateProgressStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		schoolCertificateService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        schoolCertificateService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		schoolCertificateService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
