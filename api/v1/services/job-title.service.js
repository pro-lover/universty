...
async function getAll() {
	...
	//console.log('Getting all jobTitles');

    const models = await db.JobTitle.findAll({
		paranoid: false,
		order: [
			['name', 'ASC']
		],
	...
	//console.log('Getting all jobTitles', models);
...
async function create(params, editId) {
	// validate
	if (await db.JobTitle.findOne({ where: {
		[Op.or]: [{name: params.name}, {description: params.description}]
		} }))
	{
		throw 'JobTitle already exists. Please provide a unique Name and JobTitle description.';
	}

    const model = new db.JobTitle(params);
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name && model.description == params.description ) {
		throw 'JobTitle hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.JobTitle.findOne({ where: { name: params.name } })) {
		throw 'JobTitle already exists.';
	}
...
async function getModelById(id) {
    ...
    const model = await db.JobTitle.findByPk(id, {
		paranoid: false,
		//lock: true,
		//transaction,
		/** /
    ...
    if (!model) throw 'jobTitle not found';
    return model;
}
...
async function getHistoryById(id) {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `jobTitleHistories` WHERE id = ?",
		{
			replacements: [id],
			type: db.sequelizeInstance.QueryTypes.SELECT
		}
