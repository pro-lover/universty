...
import { Account, Brand, Role } from '@app/core/models';
...
export class BrandAddEditPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit Brand Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				BrandActions.BrandActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BrandActions.BrandActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BrandActions.BrandActionTypes.MODEL_CreateFailed,
				BrandActions.BrandActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private prepForm():void {
		...
		this.form = this.formBuilder.group({
			...
			numberOfAvailableSlots: [0, Validators.required]
		});
	...
	private createRecord() {
		this.store.dispatch(
			new BrandActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: Brand ) {
		this.store.dispatch(
			new BrandActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
