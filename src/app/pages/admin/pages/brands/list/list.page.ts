...
import { Account, Brand } from '@app/core/models';
...
export class BrandListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<Brand[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(BrandSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				BrandActions.BrandActionTypes.MODEL_UpdateStatusSuccess,
				BrandActions.BrandActionTypes.MODEL_DeleteSuccess,
				BrandActions.BrandActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BrandActions.BrandActionTypes.MODEL_UpdateStatusFailed,
				BrandActions.BrandActionTypes.MODEL_DeleteFailed,
				BrandActions.BrandActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Brand[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new BrandActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new BrandActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new BrandActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		const exportArray = this.primaryData.map( (data) => {
			...
			return {
				...
				'numberOfAvailableSlots': data.numberOfAvailableSlots,
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Brands.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.BrandFilterValue.value && this.BrandFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.BrandId === this.BrandFilterValue.value
			});
		}
	...
	public sortData(sort: Sort) : void {
		...
		this.sortedData = data.sort((a, b) => {
			...
			switch (sort.active) {
				...
				case 'numberOfAvailableSlots': return this.compare(a.numberOfAvailableSlots, b.numberOfAvailableSlots, isAsc);
