...
import { Account, BriefPhase, Role } from '@app/core/models';
...
export class BriefPhaseAddEditPage implements OnInit, OnDestroy {
		...
		this.dataItemId$
			...
			.subscribe( (x) => {
				...
				this.dataItem$ = this.store.select(
					BriefPhaseSelectors.selectById(
						parseInt(x)
					)
				);
				/**/
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit BriefPhase Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_CreateFailed,
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private createRecord() {
		this.store.dispatch(
			new BriefPhaseActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: BriefPhase ) {
		this.store.dispatch(
			new BriefPhaseActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
