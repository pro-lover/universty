...
import { Account, BriefPhase } from '@app/core/models';
...
export class BriefPhaseListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<BriefPhase[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(BriefPhaseSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_UpdateStatusSuccess,
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_DeleteSuccess,
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_UpdateStatusFailed,
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_DeleteFailed,
				BriefPhaseActions.BriefPhaseActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:BriefPhase[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new BriefPhaseActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new BriefPhaseActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new BriefPhaseActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_BriefPhases.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.BriefPhaseFilterValue.value && this.BriefPhaseFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.BriefPhaseId === this.BriefPhaseFilterValue.value
			});
		}
