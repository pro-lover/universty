...
import { Account, Client,Brand } from '@app/core/models';
...
export class ClientListPage implements OnInit, OnDestroy {
	...
	public dataBrand$!: Observable<Brand[]>;
	public dataAccount$!: Observable<Account[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(ClientSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
		...
		this.dataView$
			...
			.subscribe(
				(data:Client[]) =>  {
					...
					console.warn('this.dataView$ : ic', data);
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				ClientActions.ClientActionTypes.MODEL_UpdateStatusSuccess,
				ClientActions.ClientActionTypes.MODEL_DeleteSuccess,
				ClientActions.ClientActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				ClientActions.ClientActionTypes.MODEL_UpdateStatusFailed,
				ClientActions.ClientActionTypes.MODEL_DeleteFailed,
				ClientActions.ClientActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Client[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new ClientActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ClientActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new ClientActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		const exportArray = this.primaryData.map( (data) => {
			...
			return {
				...
				'numberOfAvailableSlots': data.numberOfAvailableSlots, // get it from brand 
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Clients.xlsx');
