...
import { Account, CreativeExecution } from '@app/core/models';
...
export class CreativeExecutionListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<CreativeExecution[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(CreativeExecutionSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_UpdateStatusSuccess,
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_DeleteSuccess,
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_UpdateStatusFailed,
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_DeleteFailed,
				CreativeExecutionActions.CreativeExecutionActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:CreativeExecution[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new CreativeExecutionActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CreativeExecutionActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CreativeExecutionActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_CreativeExecutions.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.CreativeExecutionFilterValue.value && this.CreativeExecutionFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.CreativeExecutionId === this.CreativeExecutionFilterValue.value
			});
		}
