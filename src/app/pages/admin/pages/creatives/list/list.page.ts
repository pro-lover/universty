...
import { Account, Creative, Team,JobLevel,JobTitle } from '@app/core/models';
import { AccountService, AlertService,TeamService,JobLevelService,JobTitleService } from '@app/core/services';
...
export class CreativeListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<Creative[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(CreativeSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				CreativeActions.CreativeActionTypes.MODEL_UpdateStatusSuccess,
				CreativeActions.CreativeActionTypes.MODEL_DeleteSuccess,
				CreativeActions.CreativeActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				CreativeActions.CreativeActionTypes.MODEL_UpdateStatusFailed,
				CreativeActions.CreativeActionTypes.MODEL_DeleteFailed,
				CreativeActions.CreativeActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Creative[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new CreativeActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		const confirmDialog = this.dialog.open( DialogConfirmComponent, {
			data: {
				...
				message: 'Are you sure you want to delete: ' + model.description
			}
		});
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CreativeActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		const confirmDialog = this.dialog.open( DialogRestoreComponent, {
			data: {
				...
				message: 'Are you sure you want to restore this record: ' + model.description
			}
		});
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new CreativeActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Creatives.xlsx');
	...
	private initialiseTextFilters() {
		...
		this.filteredNames = this.chipCtrl.valueChanges.pipe(
			...
			map( (so: any | null) => {
				...
				return so ? this.myTextFilter('description', so) : this.masterReference_names.slice()
		}));

	}

	private myTextFilter(type:string, description: string) {
		//console.warn(description);
		switch (type) {
			case 'description':
				return this.masterReference_names.filter(so => so.description.toLowerCase().indexOf(description.toLowerCase()) === 0);
			default:
				return [];
		}
	}
	...
	public selectedTextFilter(event: MatAutocompleteSelectedEvent, type: string): void {
		...
		switch (type) {
			case 'name':
				...
				/** /
				this.sortedDataArray = [this.allData.find(x => x.id === event.option.value)];
				this.length = this.sortedDataArray.length;
				this.activeNameFilters = this.sortedDataArray;
				/**/
				this.activeNameFilters.push(this.allData.find(x => x.description === event.option.value));

				if( this.activeNameFilters.length > 0 ) {
					this.activeNameFilters.forEach((x:any)=> {
						newdb.push(this.allData.filter((y:any) => {
							return y.description === x.description
						}));
					});
				}
				break;
		...
		//console.log('selectedTextFilter['+type+']:', this.sortedDataArray);
	...
	public removeSelectedFiltered(type:string): void {
		...
		this.activeNameFilters = this.activeNameFilters.filter(x => x.description !== type);
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.CreativeFilterValue.value && this.CreativeFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.CreativeId === this.CreativeFilterValue.value
			});
		}
