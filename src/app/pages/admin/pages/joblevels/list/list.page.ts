...
import { Account, JobLevel } from '@app/core/models';
...
export class JobLevelListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<JobLevel[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(JobLevelSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				JobLevelActions.JobLevelActionTypes.MODEL_UpdateStatusSuccess,
				JobLevelActions.JobLevelActionTypes.MODEL_DeleteSuccess,
				JobLevelActions.JobLevelActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				JobLevelActions.JobLevelActionTypes.MODEL_UpdateStatusFailed,
				JobLevelActions.JobLevelActionTypes.MODEL_DeleteFailed,
				JobLevelActions.JobLevelActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:JobLevel[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new JobLevelActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new JobLevelActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new JobLevelActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_JobLevels.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.JobLevelFilterValue.value && this.JobLevelFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.JobLevelId === this.JobLevelFilterValue.value
			});
		}
