...
import { Account, JobTitle, Role } from '@app/core/models';
...
export class JobTitleAddEditPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit JobTitle Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				JobTitleActions.JobTitleActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				JobTitleActions.JobTitleActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				JobTitleActions.JobTitleActionTypes.MODEL_CreateFailed,
				JobTitleActions.JobTitleActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private createRecord() {
		this.store.dispatch(
			new JobTitleActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: JobTitle ) {
		this.store.dispatch(
			new JobTitleActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
