...
import { Account, JobTitle } from '@app/core/models';
...
export class JobTitleListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<JobTitle[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(JobTitleSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				JobTitleActions.JobTitleActionTypes.MODEL_UpdateStatusSuccess,
				JobTitleActions.JobTitleActionTypes.MODEL_DeleteSuccess,
				JobTitleActions.JobTitleActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				JobTitleActions.JobTitleActionTypes.MODEL_UpdateStatusFailed,
				JobTitleActions.JobTitleActionTypes.MODEL_DeleteFailed,
				JobTitleActions.JobTitleActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:JobTitle[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new JobTitleActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new JobTitleActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new JobTitleActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_JobTitles.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.JobTitleFilterValue.value && this.JobTitleFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.JobTitleId === this.JobTitleFilterValue.value
			});
		}
