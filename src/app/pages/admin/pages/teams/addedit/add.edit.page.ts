...
export class TeamAddEditPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		console.warn('Add/Edit Team Page ngOnDestroy');
	...
	private uiStateInfo():void {
		...
		.pipe(
			ofType(
				TeamActions.TeamActionTypes.MODEL_CreateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				TeamActions.TeamActionTypes.MODEL_UpdateSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				TeamActions.TeamActionTypes.MODEL_CreateFailed,
				TeamActions.TeamActionTypes.MODEL_UpdateFailed
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
	...
	private createRecord() {
		this.store.dispatch(
			new TeamActions.MODEL_CreateInitiated({ dataItem: {
				...this.form.value
			}})
		);
	}

	private updateRecord( data: Team ) {
		this.store.dispatch(
			new TeamActions.MODEL_UpdateInitiated({ dataItem: {
				...data
				//id: this.id.toString(),
			}})
		);
