...
export class TeamListPage implements OnInit, OnDestroy {
	...
	public dataView$!: Observable<Team[]>;
	...
	constructor(
		...
		this.dataView$ = this.store.select(TeamSelectors.selectCollection).pipe(takeUntil(this._destroy$));
	}
	...
	private uiStateInfo() {
		...
		.pipe(
			ofType(
				TeamActions.TeamActionTypes.MODEL_UpdateStatusSuccess,
				TeamActions.TeamActionTypes.MODEL_DeleteSuccess,
				TeamActions.TeamActionTypes.MODEL_RestoreSuccess
			)
		)
		.pipe(takeUntil(this._destroy$))
		.subscribe({
		...
		.pipe(
			ofType(
				TeamActions.TeamActionTypes.MODEL_UpdateStatusFailed,
				TeamActions.TeamActionTypes.MODEL_DeleteFailed,
				TeamActions.TeamActionTypes.MODEL_RestoreFailed
	...
	private initialise( data:Team[]):void {
	...
	private updateStatus( id: string, params: any ):void {
		...
		this.store.dispatch(
			new TeamActions.MODEL_UpdateStatusInitiated({
				dataId: id,
				params: params
			})
		);
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new TeamActions.MODEL_DeleteInitiated({ dataId: id }) );
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			...
			if (result === true) {
				...
				this.store.dispatch(new TeamActions.MODEL_RestoreInitiated({ dataId: id }) );
	...
	public export(): void {
		...
		/* save to file */
		XLSX.writeFile(wb, 'TBWA_Africa_Conference_All_Teams.xlsx');
	...
	public onFilterChange( filter:string ): void {
		...
		/** /
		if( this.TeamFilterValue.value && this.TeamFilterValue.value !== undefined ) {
			newdata = newdata.filter((x:any) => {
				return x.component?.container.banner.template.TeamId === this.TeamFilterValue.value
			});
		}
